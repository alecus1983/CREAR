...
if ($valido) {
  ...
  // Nota de recuperación indexada por el código del alumno
  $notas = array_column(
    (array) $score->get_recuperacion_periodo($listado->id_alumno, $id_m, $ano, $periodo),
    'R' . $periodo,
    'id_alumno'
  );

  // obtengo el conjunto de logros indexado por el código del alumno
  $logros = $score->get_logro_recuperacion_periodo($listado->id_alumno, $id_m, $ano, $periodo);

  // consultar las recuperaciones anteriores  
  ...
  foreach ($listado->id_alumno as $e) {
    ...
    //capturo la nota para este alumnos
    $nota1 = "";
    // coloco un numero vacio  si la nota es igual a cero
    // o si el alumno aun no tiene registro de recuperacion
    if (empty($notas[$e])) {
      $nota1 = "";
    } else {
      // coloco la nota asignada
      $nota1 = $notas[$e];
    }
    ...
    echo '<input type="number" step="0.1" max="5" min="0" name="R' . $periodo . '[]"  value="' . $nota1 . '"  class="form-control R" placeholder="nota" aria-label="recuperacion" aria-describedby="basic-addon1">';
    echo '</div>';

    //LOGRO
    // codigo del logro asignado, vacio si aun no tiene
    $logro1 = empty($logros[$e]) ? "" : $logros[$e];
