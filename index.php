<?php
include_once "./assets/templates/header.php";
?>

<!-- 2.Home -->
<section class="hero-slide-wrapper hero-3">
    <div class="hero-slider-3">
        <div class="single-slide bg-cover" style="background-image: url('assets/img/index/slideimgcolegio.png')">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-12 col-lg-8 mt-5 mt-lg-0 order-2 order-lg-1 text-center text-lg-left">
                        <div class="hero-contents text-white">
                            <p>Más de 20 años educando nuestra Comunidad</p>
                            <h1>¡Educación al <br> Alcance de todos!</h1>
                        </div><br><br>
                        <a target="_blank" href="https://wa.link/5gai9l" class="theme-btn">Más información<i class="fas fa-arrow-right"></i></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>


<?php
include_once "./assets/templates/baner-info.php";
?>
<!-- Home -->

<!-- 3.Services of the business -->
<br>
<br>
<br>
<br>
<!-- Test services  -->

<!-- Feature Area Start -->
<div class="feature_area section_gap">
    <div class="feature_shape circle_round"></div>
    <div class="feature_shape_2"></div>
    <div class="container">


    <div class="row mb-70">
            <div class="col-lg-6 col-12">
                <div class="section-title style-3">
                    <span>Servicios</span>
                    <p>Los mejores</p>
                    <h1>Servicios Educativos</h1>
                </div>
            </div>
            <div class="col-lg-6 mt-4 mt-lg-0 col-12 text-lg-right">
                <div class="work-process-nav"></div>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-lg-4 col-md-6 wow fadeInUp animated" data-wow-duration="1500ms" data-wow-delay="200ms">
                <a href="./colegio.php">
                <div class="single_feature">

                    <img src="assets/img/index/colegioservices.jpg" alt="features-images">
                </div>
                </a>
            </div>

            <div class="col-lg-4 col-md-6 wow fadeInUp animated" data-wow-duration="1500ms" data-wow-delay="400ms">
                <a href="./educacionciclos.php">
                <div class="single_feature">
                    <img src="assets/img/index/ciclosservices.jpg" alt="features-images">
                </div>
                </a>
            </div>

            <div class="col-lg-4 col-md-6 wow fadeInUp animated" data-wow-duration="1500ms" data-wow-delay="600ms">
                <a href="./tecnicoslaborales.php">
                <div class="single_feature single-last-item">
                    <img src="assets/img/index/tecnicosservices.jpg" alt="features-images">
                </div>
                </a>
            </div>


            

        </div>
    </div>
</div>
<!-- Feature Area End -->
<!-- Test services  -->

<br><br><br>

<!-- 4.Characteristics  -->
<section style="margin-top: -80px;" class="our-process clear-fix section-padding">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 offset-lg-2 col-12 text-center">
                <div class="section-title mb-40">
                    <p>Lo que nos hace ÚNICOS</p>
                    <h1>En santander de Quilichao</h1>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-4 col-md-6 col-12">
                <div class="single-work-process">
                    <div class="icon">
                        <i class="fal fa-gem"></i>
                        <span>01</span>
                    </div>
                    <div class="content">
                        <h3>Educación de calidad</h3>
                        <p style="font-family: sans-serif;">Contamos con un plan de educación para el trabajo y el
                            desarrollo humano.</p>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-12">
                <div class="single-work-process">
                    <div class="icon">
                        <i class="fal fa-bullseye-arrow"></i>
                        <span>02</span>
                    </div>
                    <div class="content">
                        <h3>Educación en Valores</h3>
                        <p style="font-family: sans-serif;">Contamos con formación en valores para nuestros
                            estudiantes</p>
                    </div>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-12">
                <div class="single-work-process">
                    <div class="icon">
                        <i class="fal fa-users"></i>
                        <span>03</span>
                    </div>
                    <div class="content">
                        <h3>Liderazgo</h3>
                        <p style="font-family: sans-serif;">Inspiramos a nuestros estudiantes, para transformar el
                            mundo</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="work-link d-none d-lg-block">
        <svg width="1292px" height="136px">
            <path fill-rule="evenodd" stroke="rgb(9, 107, 216)" stroke-width="2px" stroke-dasharray="8, 8" stroke-linecap="butt" stroke-linejoin="miter" opacity="0.2" fill="none" d="M1.000,131.999 C1.000,131.999 190.931,8.144 400.000,91.999 C626.535,182.860 763.243,66.417 833.000,32.000 C931.956,-16.824 1115.947,-22.085 1289.774,130.874 " />
        </svg>
    </div>
</section>
<!--  Characteristics -->

<!-- <section class="case-study-carousel-wrapper style-2">
    <div class="container">
        <div class="row mb-70">
            <div class="col-lg-6 col-12">
                <div class="section-title style-3">
                    <span>Servicios</span>
                    <p>Descubre nuestros</p>
                    <h1>Cursos y seminarios</h1>
                </div>
            </div>
            <div class="col-lg-6 mt-4 mt-lg-0 col-12 text-lg-right">
                <div class="work-process-nav"></div>
            </div>
        </div>

        <div class="case-study-items owl-carousel text-center">
         
            <a href="./colegio.php">
                <div class="single-case-item">
                    <div class="p-4 case-thumb bg-cover hover-service1">
                    </div>
                </div>
            </a>
 

          
            <a href="./cursoscortos.php">
                <div class="single-case-item">
                    <div class="case-thumb bg-cover hover-service2">
                    </div>
                </div>
            </a>
           

           
            <a href="./tecnicoslaborales.php">
                <div class="single-case-item">
                    <div class="case-thumb bg-cover hover-service3">
                    </div>
                </div>
            </a>
   

            <a href="./educacionciclos.php">
                <div class="single-case-item">
                    <div class="case-thumb bg-cover hover-service4">
                    </div>
                </div>
            </a>
   
        </div>
    </div>
</section>
<br><br><br> -->
<!-- Services of the business -->

<!-- Ident -->
<section class="container">
    <div class="row">
        <div class="col-12 col-lg-12 text-center">
            <div class="section-title style-3">
                <span>CREATIVO</span>
                <p>Conoce</p>
                <h1>Nuestra Misión</h1>
            </div>
        </div>
    </div>
</section>
<br><br>
<div class="container">
    <p style="font-family: sans-serif;    width: 80%;
        margin: auto;" class="text-center">En la Fundación Instituto Mundo Creativo formamos niñas, niños, jóvenes y adultos en educación, posibilitando su desarrollo integral, potenciando sus capacidades individuales mediante la adquisición de competencias para que construyan el conocimiento y transformen su realidad sociocultural, a través de un modelo pedagógico transformador que les permita ser gestores y protagonistas de su proyecto de vida desde la innovación educativa.</p>
</div>
<br><br><br>
<section class="container">
    <div class="row">
        <div class="col-12 col-lg-12 text-center">
            <div class="section-title style-3">
                <span>CREATIVO</span>
                <p>Conoce</p>
                <h1>Nuestra Visión</h1>
            </div>
        </div>
    </div>
</section>
<br><br>
<div class="container">
    <p style="font-family: sans-serif;    width: 80%;
        margin: auto;" class="text-center">Seremos reconocidos por la integridad de la propuesta educativa centrada en
        la formación del ser y el desarrollo de competencias que responden a los estándares de calidad, mediante
        procesos pedagógicos que reconocen la individualidad y promuevan la autonomía de la persona, con principios
        y valores sociales. Apoyados por una comunidad educativa comprometida con el mejoramiento institucional de
        su entorno.</p>
</div>
<!-- Ident -->
<br>
<br>
<!-- REDES SOCIALES -->
<div id="Contactos" style="margin-top: -50px;" class='flx'>
    <div class='demo-content'>
        <div class="nshare">
            <a target="_blank" class="nshare-item nshare-fb" href="https://www.facebook.com/imcreativo.edu.co">
                <i class="fab fa-facebook"></i>
            </a>
            <a target="_blank" class="nshare-item nshare-tt" href="https://www.instagram.com/imcreativo.edu.co/">
                <i class="fab fa-instagram"></i>
            </a>
            <a target="_blank" class="nshare-item nshare-ws" href="https://wa.link/5gai9l">
                <i class="fab fa-whatsapp"></i>
            </a>

        </div>
    </div>
</div>
<br>

<!--Btn whatsapp-->
<div class=" float2">
    <div class="nshare">
        <a class="nshare-item nshare-ws" href="https://wa.link/5gai9l">
            <i class="fab fa-whatsapp"></i>
        </a>
    </div>
</div>
<!--Btn whatsapp-->
<!-- FIN REDES SOCIALES -->

<?php
include_once "./assets/templates/footer.php";
?>